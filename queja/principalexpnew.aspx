...
<body class="width:100%">
    <span class="ir-arriba">
        <img class="manImg" src="imagenes/arrow_up.png" width="70px" height="70px"></img></span>
...
<script type="text/javascript">
    ...
    function GuardarQuejaExp() {
        ...
        if (invalidItems.length > 0) {
            // 1. Definimos la lista de todos los campos que estás validando
            const camposAValidar = [
                'Pedido', 'ProdCve', 'Producto', 'OrdProd', 'Tarima', 'Lote', 'Fecha', 'CveProv',
                'Proveedor', 'CveRch', 'Rancho', 'CveTbl', 'Tabla', 'Responsable', 'Area', 'Recibido',
                'Rechazadas', 'Producidas', 'Porcentaje', 'Unidad', 'Tipo', 'Variedad', 'CNTE',
                'Foli', 'Prob', 'Devo', 'Merm', 'Boni', 'Noap', 'Mone', 'Cons'
            ];

            // 2. Construimos el HTML dinámicamente
            const listHtml = `
        <p>Los siguientes productos tienen campos obligatorios vacíos:</p>
        <div style="text-align: left; max-height: 250px; overflow-y: auto; background: #f8f9fa; padding: 15px; border-radius: 8px; font-size: 14px;">
            ${invalidItems.map(p => {
                // Buscamos cuáles de los campos obligatorios están vacíos en este producto específico
                const camposVacios = camposAValidar.filter(campo => !p[campo]);

                return `
                    <div style="margin-bottom: 12px; border-bottom: 1px dashed #ccc; padding-bottom: 8px;">
                        <strong>📦 ${p.Producto || 'Producto sin nombre'}</strong> (Lote: ${p.Lote || 'N/A'})
                        <div style="color: #dc3545; margin-top: 4px; padding-left: 15px;">
                            ❌ Faltan: <u>${camposVacios.join(', ')}</u>
                        </div>
                    </div>
                `;
            }).join('')}
    ...
    `;
            ...
            // 3. Lanzamos la alerta de SweetAlert
            Swal.fire({
                title: "Información Incompleta",
                html: listHtml,
                icon: "error",
                customClass: {
                    popup: 'format-pre' // Opcional, por si quieres darle más estilos
                }
            });
