...
namespace queja
{
    public partial class upload_ver : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.lblAccioncorrectiva.Text = this.Session["accion"].ToString();
        }

        protected void btnSubir_Click(object sender, EventArgs e)
        {
            if (this.lblAccioncorrectiva.Text == "-")
                this.Response.Write("<script>alert('Cierre la ventana y guarde primero la accion correctiva despues intente volver a subir las imagenes')</script>");
            else if (this.img1.HasFile && this.img1.PostedFile.ContentLength > 4194304)
          ...
          "pdf"
                ...
                if (this.img1.HasFile)
                {
                    if (this.img1.FileName.Contains(".pdf"))
                        this.img1.SaveAs(str1 + "1_" + this.lblQueja.Text + "_" + this.lblAccioncorrectiva.Text + ".pdf");
                    else
                        upload_ver.ScaleImage((System.Drawing.Image)new Bitmap(this.img1.PostedFile.InputStream), 405).Save(str1 + "1_" + this.lblQueja.Text + "_" + this.lblAccioncorrectiva.Text + ".jpg", ImageFormat.Jpeg);
                }
                if (this.img2.HasFile)
                {
                    if (this.img2.FileName.Contains(".pdf"))
                        this.img2.SaveAs(str1 + "2_" + this.lblQueja.Text + "_" + this.lblAccioncorrectiva.Text + ".pdf");
                    else
                        upload_ver.ScaleImage((System.Drawing.Image)new Bitmap(this.img2.PostedFile.InputStream), 405).Save(str1 + "2_" + this.lblQueja.Text + "_" + this.lblAccioncorrectiva.Text + ".jpg", ImageFormat.Jpeg);
                }
                if (this.img3.HasFile)
                {
                    if (this.img3.FileName.Contains(".pdf"))
                        this.img3.SaveAs(str1 + "3_" + this.lblQueja.Text + "_" + this.lblAccioncorrectiva.Text + ".pdf");
                    else
                        upload_ver.ScaleImage((System.Drawing.Image)new Bitmap(this.img3.PostedFile.InputStream), 405).Save(str1 + "3_" + this.lblQueja.Text + "_" + this.lblAccioncorrectiva.Text + ".jpg", ImageFormat.Jpeg);
                }
